# ---------------------------------------------------------------------------
# Vars for ExaScale (jde)
# Ported from oracle-ebs-framework/variables.tf, EBS names -> jde.
# "jde-" prefixes avoid collisions because jde shares the EBS GCP project.
# ---------------------------------------------------------------------------

variable "oracle_jde_exascale" {
  description = "Whether to deploy the Oracle jde ExaScale environment"
  type        = bool
  default     = false
}
...
variable "odb_subnet_id" {
  ...
  type        = string
  default     = "oracle-jde-toolkit-odb-subnet"
}
...
variable "exascale_storage_vault_id" {
  ...
  type        = string
  default     = "jde-exascale-db-storage-vault"
}
...
variable "exascale_jde_machine_type" {
  description = "Machine type for the ExaScale jde application VM"
  type        = string
  default     = "e2-highmem-8"
}

variable "exascale_jde_boot_disk_size" {
  description = "Boot disk size (GB) for the ExaScale jde application VM"
  type        = number
  default     = 512
}

variable "exascale_jde_boot_disk_type" {
  description = "Boot disk type for the ExaScale jde application VM"
  type        = string
  default     = "pd-balanced"
}

variable "exascale_jde_boot_disk_auto_delete" {
