...
resource "tls_private_key" "exadb_ssh_key" {
  count     = var.oracle_jde_exascale ? 1 : 0
  algorithm = "RSA"
  rsa_bits  = 4096
}
    ...
    command     = <<EOT
      ...
      oci network nsg rules add \
        ...
        --security-rules '[
          ...
          "sourceType": "CIDR_BLOCK",
          "tcpOptions": {
            "destinationPortRange": {"max": 5150, "min": 5150}
            }
          },
          {
            "direction": "INGRESS",
...
resource "null_resource" "exascale_configure_and_upload" {
  ...
  triggers = {
    vm_id           = try(google_compute_instance.exascale_jde[0].id, "")
  ...
  depends_on = [
    ...
    google_compute_instance.exascale_jde
  ]
...
EOF
      ...
      while ! gcloud compute scp ./exascale_outputs.yaml \
        ${try(google_compute_instance.exascale_jde[0].name, "placeholder")}:/tmp/exascale_outputs.yaml \
        --zone="${var.zone}" \
        --project="${var.project_id}" \
        --tunnel-through-iap \
        --quiet; do
